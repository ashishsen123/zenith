...
function poloray_optionsframework_add_admin() {
    global $query_string;

    $themename = poloray_get_option('of_themename');
    $shortname = poloray_get_option('of_shortname');

    if (isset($_REQUEST['page']) && $_REQUEST['page'] == 'optionsframework') {
        if (isset($_REQUEST['of_save']) && 'reset' == $_REQUEST['of_save']) {
            $options = poloray_get_option('of_template');
            poloray_reset_options($options, 'optionsframework');
            header("Location: admin.php?page=optionsframework&reset=true");
            die;
        }
    }
    $of_page = add_theme_page($themename, 'Theme Options', 'edit_theme_options', 'optionsframework', 'poloray_optionsframework_options_page', 'div');

    // Add framework functionaily to the head individually
    add_action("admin_print_scripts-$of_page", 'poloray_load_only');
}

add_action('admin_menu', 'poloray_optionsframework_add_admin');
/* ----------------------------------------------------------------------------------- */
/* Options Framework Reset Function - of_reset_options */
/* ----------------------------------------------------------------------------------- */

function poloray_reset_options($options, $page = '') {
    global $wpdb;
    $count = 0;
    ...
    if ($page == 'optionsframework') {
        poloray_delete_option('of_options');
    }
}
...
function poloray_optionsframework_options_page() {
    $options = poloray_get_option('of_template');
    $themename = poloray_get_option('of_themename');
    ?>
    <div class="trail-notify">
        <h1><?php _e('Upgrade to Poloray Theme PRO!', 'poloray'); ?></h1>
    <?php _e('<p>You are Using the Lite Version of Poloray Theme, Upgrade to Pro to get more Home Page Image Slider, Added HomePage Sections, Gallery, Contact Page Template, Premium Support and more.</p>
        <a href="http://www.inkthemes.com/wp-themes/poloray-responsive-wordpress-theme/" target="blank">Click Here to Upgrade to Poloray Theme PRO !</a>', 'poloray'); ?>
    </div>
    <div class="wrap" id="of_container">
        <div id="of-popup-save" class="of-save-popup">
            <div class="of-save-save"><?php _e('Options Updated', 'poloray'); ?></div>
        </div>
        <div id="of-popup-reset" class="of-save-popup">
            <div class="of-save-reset"><?php _e('Options Reset', 'poloray'); ?></div>
        </div>
        <form action="" enctype="multipart/form-data" id="ofform">
    <?php wp_nonce_field('theme-update-option'); ?>
            <div id="header">
                <div class="logo">
                    <h2><?php echo $themename; ?><?php _e('Options', 'poloray'); ?></h2>
                </div>
                ...
                <div class="clear"></div>
                        ...
                        $return = poloray_optionsframework_machine($options);
                        ?>
            <div id="main">
                <div id="of-nav">
                    <ul>
...
function poloray_load_only() {
    ...
    wp_enqueue_script('jquery-tiny_mce');

    function of_admin_head() {
        //echo '<link rel="stylesheet" type="text/css" href="'. get_template_directory_uri().'/functions/admin-style.css" media="screen" />';
        wp_enqueue_style('poloray-admin-style', get_template_directory_uri() . "/functions/admin-style.css", '', '', 'screen');
        wp_enqueue_style('poloray-color-picker', get_template_directory_uri() . "/functions/css/colorpicker.css", '', '', 'screen');
        wp_register_script('jquery-input-mask', get_template_directory_uri() . '/functions/js/colorpicker.js', array('jquery'));
        // COLOR Picker 
        ?>
        ...
        foreach ($options as $option) {
            if ($option['type'] == 'color' OR $option['type'] == 'typography' OR $option['type'] == 'border') {
                if ($option['type'] == 'typography' OR $option['type'] == 'border') {
                    $option_id = $option['id'];
                    $temp_color = poloray_get_option($option_id);
                    $option_id = $option['id'] . '_color';
                    $color = $temp_color['color'];
                } else {
                    $option_id = $option['id'];
                    $color = poloray_get_option($option_id);
                }
                ?>
...
/* ----------------------------------------------------------------------------------- */
/* Ajax Save Action - poloray_ajax_callback */
/* ----------------------------------------------------------------------------------- */
add_action('wp_ajax_of_ajax_post_action', 'poloray_ajax_callback');

function poloray_ajax_callback() {
    ...
    if ($save_type == 'upload') {
        ...
        poloray_update_option($clickedID, $uploaded_file['url']);
        ...
        foreach ($options as $option_array) {
            $id = $option_array['id'];
            $old_value = poloray_get_option($id);
            $new_value = '';
                ...
                if (is_array($type)) {
                    foreach ($type as $array) {
                        if ($array['type'] == 'text') {
                            ...
                            poloray_update_option($id, stripslashes($new_value));
                        }
                    }
                } elseif ($new_value == '' && $type == 'checkbox') { // Checkbox Save
                    poloray_update_option($id, 'false');
                } elseif ($new_value == 'true' && $type == 'checkbox') { // Checkbox Save
                    poloray_update_option($id, 'true');
                    ...
                    foreach ($option_options as $options_id => $options_value) {
                        ...
                        if (!isset($output[$multicheck_id])) {
                            poloray_update_option($multicheck_id, 'false');
                        } else {
                            poloray_update_option($multicheck_id, 'true');
                        }
                    }
                    ...
                    poloray_update_option($id, $typography_array);
                } elseif ($type == 'border') {
                    ...
                    poloray_update_option($id, $border_array);
...
function poloray_optionsframework_machine($options) {
    ...
    foreach ($options as $value) {
        ...
        switch ($value['type']) {
            ...
            case 'text':
                $val = $value['std'];
                $std = poloray_get_option($value['id']);
                if ($std != "") {
                    $val = $std;
                }
            ...
            case 'select':
                ...
                $select_value = poloray_get_option($value['id']);
            ...
            case "upload":
                $value['std'] = '';
                if (isset($value['std'])) {
                    $output .= poloray_optionsframework_uploader_function($value['id'], $value['std'], null);
                }
                break;
            case "upload_min":

                $output .= poloray_optionsframework_uploader_function($value['id'], $value['std'], 'min');

                break;
            case "color":
                $val = $value['std'];
                $stored = poloray_get_option($value['id']);
                if ($stored != "") {
                    $val = $stored;
                }
            ...
            case "typography":
                ...
                $default = $value['std'];
                $typography_stored = poloray_get_option($value['id']);
            ...
            case "border":
                ...
                $default = $value['std'];
                $border_stored = poloray_get_option($value['id']);
        ...
        if (is_array($value['type'])) {
            foreach ($value['type'] as $array) {
                ...
                $id = $array['id'];
                $std = $array['std'];
                $saved_std = poloray_get_option($id);
                if ($saved_std != $std) {
                    $std = $saved_std;
                }
                $meta = $array['meta'];
...
/* ----------------------------------------------------------------------------------- */
/* OptionsFramework Uploader - poloray_optionsframework_uploader_function */
/* ----------------------------------------------------------------------------------- */

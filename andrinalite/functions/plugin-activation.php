...
if (!class_exists('TGM_Plugin_Activation')) {
    ...
    class TGM_Plugin_Activation {
        ...
        public function admin_menu() {
            ...
            foreach ($this->plugins as $plugin) {
                if (!is_plugin_active($plugin['file_path'])) {
                  add_theme_page( $this->strings['page_title'], $this->strings['menu_title'], 'edit_theme_options', $this->menu, array( &$this, 'install_plugins_page' ) );
                    break;
                }
            }
        }
        ...
        protected function do_plugin_install() {
            ...
            /** Checks for actions from hover links to process the activation */ elseif (isset($_GET[sanitize_key('plugin')]) && ( isset($_GET[sanitize_key('tgmpa-activate')]) && 'activate-plugin' == $_GET[sanitize_key('tgmpa-activate')] )) {
        ...
        public function notices() {
            ...
            foreach ($this->plugins as $plugin) {
                ...
                if (is_plugin_active($plugin['file_path'])) {
                    ...
                    /** No minimum version specified, so iterate over the plugin */ else {
                        continue;
                    }
                }
                ...
                /** Installed but not active */ elseif (is_plugin_inactive($plugin['file_path'])) {
                    ...
                    /** Need higher privileges to activate the plugin */ else {
            ...
            if (!get_user_meta(get_current_user_id(), 'tgmpa_dismissed_notice', true)) {
                ...
                if (!empty($message)) {
                   ...
                   /*** plugin activation when already present in plugin list ***/
					$path = 'formget-contact-form/index.php';
                    $show_activate_link = $activate_link ? '<a href="'.wp_nonce_url(admin_url('plugins.php?action=activate&plugin='.$path), 'activate-plugin_'.$path).'">' . translate_nooped_plural($this->strings['activate_link'], $activate_link_count, 'tgmpa') . '</a>' : '';
    ...
    class TGMPA_List_Table extends WP_List_Table {
        ...
        public function column_plugin($item) {
            ...
            if (!isset($installed_plugins[$item['file_path']])) {
                $actions = array(
                    'install' => sprintf(
                            '<a href="%1$s" title="Install %2$s">Install</a>', wp_nonce_url(
                                    add_query_arg(
                                            array(
            ...
            /** We need to display the 'Activate' hover link */ elseif (is_plugin_inactive($item['file_path'])) {
                $actions = array(
                    'activate' => sprintf(
                            '<a href="%1$s" title="Activate %2$s">Activate</a>', add_query_arg(
                                    array(
                        'page' => TGM_Plugin_Activation::$instance->menu,
                        'plugin' => $item['slug'],
                        'plugin_name' => $item['sanitized_plugin'],
        ...
        public function process_bulk_actions() {
            ...
            if ('tgmpa-bulk-install' === $this->current_action()) {
                ...
                /** Information has been passed via $_GET */ else {
    ...
    if (!class_exists('TGM_Bulk_Installer')) {
        ...
        class TGM_Bulk_Installer extends WP_Upgrader {
            ...
            public function run($options) {
                ...
                /** The plugin install is successful */ else {
                ...
                if (TGM_Plugin_Activation::$instance->is_automatic) {
                    ...
                    /** The plugin activation is successful */ else {

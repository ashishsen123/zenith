...
$socialProfiles = array('Facebook', 'Twitter', 'Google Plus', 'RSS', 'Email', 'Youtube', 'Vimeo', 'LinkedIn', 'Flickr');
...
function widget( $args, $instance ) {
		...
		<ul class="social">
			...
			foreach ($socialProfiles as $item)
			{
				...
				if ($instance[$id] && $instance[$id] != '') {
				    ...
				    echo '<li><a class="'.$id.'" href="'.$instance[$id].'" rel="external,nofollow" title="'.$instance[$iditem].'"><img src="'. get_bloginfo('template_directory') .'/images/icons/social_widget/'.strtolower($item).'.png" alt="'.$instance[$iddesc]. '" />'.$instance[$iditem].'<span>'.$instance[$iddesc].'</a></li>'; 
				}
			} 
			?>
  		</ul>
    ...
    ?>
		...
		foreach ($socialProfiles as $item)
		{
			...
			?>
 	 		<p>
				<img style="float: left; margin:0 8px 0 0;" src="<?php echo bloginfo('template_directory') ?>/images/icons/social_widget/<?php echo $id; ?>.png" />
			...
			<p style="margin-left:33px;"> 
				...
				<input type="text"  id="<?php echo $this->get_field_id( $idtitle ); ?>" name="<?php echo $this->get_field_name( $idtitle ); ?>" value="<?php echo $instance[$idtitle]; ?>" size="23" /><br/>

 				<label for="<?php echo $this->get_field_id( $iddesc ); ?>">Description</label>
				<input type="text"  id="<?php echo $this->get_field_id( $iddesc ); ?>" name="<?php echo $this->get_field_name( $iddesc ); ?>" value="<?php echo $instance[$iddesc]; ?>" size="23" /><br/>
			</p> 

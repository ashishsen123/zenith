...
class ZOOM_Portfolio {
    ...
    public function locate_template( $type, $selected = false ) {
        ...
        if ( $selected ) {
            /* Child theme. */
            $paths = glob( STYLESHEETPATH . "/portfolio/$type*.php" );
            foreach ( $paths as $path ) {
                $template_info = WPZOOM::get_file_data( $path, array( 'Portfolio Style' ) );
            ...
            /* Parent theme. */
            $paths = glob( TEMPLATEPATH . "/portfolio/$type*.php" );
            foreach ( $paths as $path ) {
                $template_info = WPZOOM::get_file_data( $path, array( 'Portfolio Style' ) );
        ...
        /* Child theme. */
        if ( file_exists( STYLESHEETPATH . "/portfolio/$type.php" ) ) {
            return STYLESHEETPATH . "/portfolio/$type.php";
        }

        /* Parent theme. */
        if ( file_exists( TEMPLATEPATH . "/portfolio/$type.php" ) ) {
            return TEMPLATEPATH . "/portfolio/$type.php";
        }
    ...
    public static function get_available_templates( $type ) {
        ...
        foreach ( $paths as $path ) {
            $file_data = WPZOOM::get_file_data( $path, array( 'Portfolio Style' ) );
            $names[] = $file_data[0];
        }

...
//Load languages file
    load_theme_textdomain('toommorel', get_template_directory() . '/languages');
    $locale = get_locale();
    $locale_file = get_template_directory() . "/languages/$locale.php";
    if (is_readable($locale_file))
        get_template_part($locale_file);
    add_theme_support('automatic-feed-links');
    register_nav_menu('custom_menu', __('Main Menu', 'toommorel'));
}
...
function inkthemes_breadcrumbs() {
    ...
    if (get_query_var('paged')) {
        if (is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author())
            echo ' (';
        echo __('Page', 'toommorel') . ' ' . get_query_var('paged');
        if (is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author())
            echo ')';
    }
    echo '</div>';
...
// Retrieves tag list of current post, separated by commas.
    ...
    if ($tag_list) {
        $posted_in = __('This entry was posted in %1$s and tagged %2$s. Bookmark the <a href="%3$s" title="Permalink to %4$s" rel="bookmark">permalink</a>.', 'toommorel');
    } elseif (is_object_in_taxonomy(get_post_type(), 'category')) {
        $posted_in = __('This entry was posted in %1$s. Bookmark the <a href="%3$s" title="Permalink to %4$s" rel="bookmark">permalink</a>.', 'toommorel');
    } else {
        $posted_in = __('Bookmark the <a href="%3$s" title="Permalink to %4$s" rel="bookmark">permalink</a>.', 'toommorel');
    }
...
// Area 1, located at the top of the sidebar.
    register_sidebar(array(
        'name' => __('Primary Widget Area', 'toommorel'),
        'id' => 'primary-widget-area',
        'description' => __('The primary widget area', 'toommorel'),
        'before_widget' => '',
        'after_widget' => '',
        'before_title' => '<h4>',
        'after_title' => '</h4>',
...
// Area 2, located below the Primary Widget Area in the sidebar. Empty by default.
    register_sidebar(array(
        'name' => __('Secondary Widget Area', 'toommorel'),
        'id' => 'secondary-widget-area',
        'description' => __('The secondary widget area', 'toommorel'),
        'before_widget' => '',
        'after_widget' => '',
        'before_title' => '<h4">',
        'after_title' => '</h4>',
